...
import VacanciesCreateList from "./VacanciesCreateList"
...
const AddVacancies = () => {
  ...
  const DelateCard = useCallback(async (data) => {
    ...
    try {
      if (data) {
        const response = await request('/api/vacancies/delete', 'POST', {_id:data},{Authorization: `Bearer ${token}`})
  ...
  return (
    <div className="pt-5 mt-3">
      ...
      <ToastContainer
        ...
        pauseOnHover
      />
      <VacanciesCreateList path={"/api/vacancies"} _id={cardId} update={fetchData} setId={setCardId}/>
          ...
          <div className="row ">
            <div className="">
              ...
              <button
                type="button"
                className={`btn  m-3  searchFilterButton ${
                  filterType === "Вакансии в литве".toLowerCase() ? "active" : ""
                }`}
                onClick={() => handleFilterChange("вакансии  в литве".toLowerCase())}
              >
                Вакансии в Литве
              </button>
              <button
                type="button"
                className={`btn  m-3  searchFilterButton ${
                  filterType === "Вакансии в латвии".toLowerCase() ? "active" : ""
                }`}
                onClick={() => handleFilterChange("Вакансии в латвии".toLowerCase())}
              >
                Вакансии в Латвии
              </button>
              <button
                type="button"
                className={`btn  m-3  searchFilterButton ${
                  filterType === "Обучение в Германии".toLowerCase() ? "active" : ""
                }`}
                onClick={() => handleFilterChange("Обучение в Германии".toLowerCase())}
              >
                Обучение в Германии
              </button>
            </div>
